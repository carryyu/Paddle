...
void blha(const phi::GPUContext &dev_ctx,
  ...
  VLOG(1) << "batch_size: " << batch_size << " num_head: " << num_head
  ...
  if (shift != nullptr) {
    ...
    DispatchBLHA<T>(
        dev_ctx, qkv_tensor, params, num_head, dim_head, out_tensor);
  }
}
...
#pragma unroll
    ...
    phi::Store<T, VecSize>(src_vec, &qkv_out[linear_index]);
  }
}
...
__global__ void VariableLengthRotaryKernel(
  ...
  for (int64_t linear_index = global_thread_idx * VecSize,
    ...
    phi::Load<int, VecSize>(&qkv[linear_index], &src_vec);
...
#pragma unroll
    ...
    phi::Store<T, VecSize>(bias_vec, &qkv_out[linear_index]);
  }
}
