  ...
  if (tid == 0) {
    ...
    for (int i = 0; i < TopPBeamTopK; i++) {
      ...
      if (!flag) {
        ...
        sum_prob += val;
        float random_ratio = exponential_transform(curand_uniform(dev_curand_states + bid), 1.0f);
        // printf("random_ratio: %f\n", random_ratio);

        // for (int t = 0; t < 5; t++) {
        //   float tmp_random_ratio = curand_uniform(dev_curand_states + bid);
...
void TopPSamplingKernel(const Context& dev_ctx,
  ...
  if (infer_seed) {
    ...
    if (seed == -1) {
      ...
      setup_kernel<<<1, 256, 0, cu_stream>>>(dev_curand_states, seed, offset, bs, need_batch_random);
    } else {
      setup_kernel<<<1, 256, 0, cu_stream>>>(dev_curand_states, seed, offset, bs, need_batch_random);
    }
  }
