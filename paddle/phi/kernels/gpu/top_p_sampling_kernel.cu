...
#pragma unroll
  for (int offset = 16; offset > 0; offset >>= 1) {
    T tmp_val =
        paddle::platform::CudaShuffleDownSync(FINAL_MASK, input.v, offset);
    int tmp_id =
        paddle::platform::CudaShuffleDownSync(FINAL_MASK, input.id, offset);
...
__global__ void topp_sampling(T* sorted_probs,
  ...
  if (!skip) {
    ...
    if (lane_id == active_lane_id) {
      ...
      // i_activate); for (int i=0; i < active_lane_id; i++) {
...
void TopPSamplingKernel(const Context& dev_ctx,
  ...
  int64_t* out_ptr = dev_ctx.template Alloc<int64_t>(out);

  DenseTensor topp_probs;
  topp_probs.Resize(phi::make_ddim({bs, 1}));
  dev_ctx.template Alloc<T>(&topp_probs);
  ...
  // DenseTensor need_sorted;
  // need_sorted.Resize(phi::make_ddim({1}));
  // dev_ctx.template Alloc<int>(&need_sorted);
  // set_sorted_num<<<1, 1, 0, cu_stream>>>(need_sorted.data<int>(), bs);
  ...
  switch (BlockSize) {
    FIXED_BLOCK_DIM(
        ...
        KeMatrixTopPBeamTopK<T, 1, 1, kBlockDim>
        <<<bs, kBlockDim, 0, cu_stream>>>(
            reinterpret_cast<T*>(const_cast<T*>(x.data<T>())),
            reinterpret_cast<T*>(const_cast<T*>(ps.data<T>())),
            out_ptr,
            reinterpret_cast<T*>(topp_probs.data<T>()),
            vocab_size,
            dev_curand_states,
            count_iter.data<int>(),
  ...
  // DenseTensor need_sorted_cpu;
  // phi::CPUPlace cpu;
  // phi::Copy(dev_ctx, need_sorted, cpu, false, &need_sorted_cpu);
  // if (*(need_sorted_cpu.data<int>()) != 0) {
  //   size_t temp_storage_bytes = 0;
  //   if (bs == 1) {
  ...
  //   temp_storage.Resize(phi::make_ddim({temp_size}));
  //   dev_ctx.template Alloc<uint8_t>(&temp_storage);
  //   cub::DeviceRadixSort::SortPairsDescending(temp_storage.data<uint8_t>(),
  //                                             temp_storage_bytes,
  //                                             x.data<T>(),
  //                                             sorted_out.data<T>(),
  //                                             inds_input.data<int64_t>(),
  ...
  //                       count_iter_begin.data<int>()));
  //   default:
  //     PD_THROW("the input data shape has error in the topp_sampling
  //     kernel.");
  // }
  // }
  return;
