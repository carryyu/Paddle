...
__inline__ __device__ float tanh_opt(float x) {
  ...
  //     return copysignf_pos((1.0f - __expf(exp_val)) / (__expf(exp_val)
  //     + 1.0f), x);
  // #endif
}
...
struct GeluFunctor {
  inline __host__ __device__ T operator()(const T x) const {
    ...
    // U casted_x_mod = 0.5f * (1.0f + tanh_opt((0.7978845608028654f * (casted_x
    // + 0.044715f * casted_pow3)))); return static_cast<T>(casted_x_mod *
    // casted_x);
    T val_pow3 = x * x * x;
...
//     const U out = (casted_x * static_cast<U>(0.5) * (static_cast<U>(1) +
//     temp)); return static_cast<T>(out);
//   }
// };
...
void LaunchDropoutActBias(Functor act_functor,
  ...
  if (cols % VecSize == 0) {
    ...
    int threads = std::max(static_cast<int32_t>(32), std::min(tmp_cols, 128));
    const auto blocks_x =
