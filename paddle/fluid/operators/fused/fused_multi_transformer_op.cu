 ...
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    if (remove_padding && !time_step) {
      ...
      auto *d_token_num =
          dev_ctx.Alloc<int>(d_token_tensor, d_token_tensor->numel());
      // alloc the max size of padding_offset_tensor
      padding_offset_tensor->Resize({bsz_seq});
      dev_ctx.Alloc<int>(padding_offset_tensor, padding_offset_tensor->numel());
      ...
      dev_ctx.Alloc<T>(x_remove_padding, x_remove_padding->numel());
      ...
      token_num = bsz_seq;
      d_token_tensor = nullptr;
      padding_offset_tensor = nullptr;
      x_remove_padding = nullptr;
      sequence_lengths = nullptr;
    }
    auto *padding_offset_data =
        remove_padding ? padding_offset_tensor->data<int>() : nullptr;
    ...
    phi::DenseTensor tmp_out, tmp_out_for_rm_padding;
    tmp_out.Resize({{token_num, dim_embed}});
    auto *tmp_out_data =
        dev_ctx.Alloc<T>(&tmp_out, tmp_out.numel() * sizeof(T));
    if (remove_padding && !time_step) {
      tmp_out_for_rm_padding.Resize({{token_num, dim_embed}});
      dev_ctx.Alloc<T>(&tmp_out_for_rm_padding,
                       tmp_out_for_rm_padding.numel() * sizeof(T));
    }
    const T *x_data;
    if (remove_padding && !time_step) {
    ...
    for (int i = 0; i < layers; ++i) {
      ...
      if (remove_padding && !time_step && i == layers - 1) {
        if (pre_layer_norm) {
          if (layers & 1) {
            // buf1 as out
