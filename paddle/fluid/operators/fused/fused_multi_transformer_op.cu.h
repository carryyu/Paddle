...
public:
  FFNGluHelper(const phi::GPUContext &dev_ctx, 
               ...
               int hid_dim, 
               int dim_ffn, 
               int dim_embed): dev_ctx_(dev_ctx), act_method_(act_method), token_num_(token_num), hid_dim_(hid_dim), dim_ffn_(dim_ffn), dim_embed_(dim_embed) {}
  ...
  void Compute(const phi::DenseTensor *input,
    ...
    // output's shape [token_num, hid_dim], bias_out's shape [token_num, dim_ffn]
    VLOG(0) << "in geglu";
    auto ffn_linear_compute = AttnMatMul<T>(
        dev_ctx_, false, false, token_num_, dim_ffn_, dim_embed_, true);
    ffn_linear_compute.ComputeForward(
    ...
    switch (hid_dim_ % PackSize) {
      case 0:
        ...
        ActFFNGlu<T, GeluFunctor<T>, PackSize><<<grid_size, block_size, 0, dev_ctx_.stream()>>>(bias_out->data<T>(), output->data<T>(), gelu, token_num_, hid_dim_);
        break;
      default:
        block_size = min(hid_dim_, 256);
        ActFFNGlu<T, GeluFunctor<T>, 1><<<grid_size, block_size, 0, dev_ctx_.stream()>>>(bias_out->data<T>(), output->data<T>(), gelu, token_num_, hid_dim_);
        break;
    }
  }
private:
